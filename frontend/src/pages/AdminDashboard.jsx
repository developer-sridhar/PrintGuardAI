    ...
    return (
        <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
            <div className="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
                <div>
                    <h1 className="text-2xl sm:text-3xl font-bold text-white mb-2 flex items-center gap-3 flex-wrap">
                ...
                <button onClick={() => fetchAdminData()} className="p-2.5 bg-dark-900 border border-zinc-800 rounded-xl text-zinc-400 hover:text-white transition-colors self-end sm:self-auto">
            ...
            {/* Tabs */}
            <div className="flex overflow-x-auto hide-scrollbar gap-1 mb-6 bg-dark-950 p-1 rounded-xl border border-zinc-800 w-full sm:w-fit shrink-0">
                {['users', 'jobs', 'logs'].map(tab => (
                    <button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 py-2 sm:py-1.5 text-xs sm:text-[10px] font-bold rounded-lg transition-all capitalize whitespace-nowrap ${activeTab === tab ? 'bg-amber-400 text-dark-950 shadow-lg shadow-amber-400/20' : 'text-zinc-500 hover:text-zinc-300'}`}>
